...
type QueryResult struct {
	...
	// Result type markers
	IsDone           bool // Query completed, no streaming results
	IsRowStream      bool // Row streaming results
	IsDocumentStream bool // Document streaming results

	// Row streaming results
	ColumnNames []string        // Column names
	Rows        [][]interface{} // Row data

	// Document streaming results
...
func (db *Database) BeginTransaction(ctx context.Context, txType TransactionType) (*Transaction, error) {
	...
	// Try to create transaction stream, including authentication error retry mechanism
...
func (tx *Transaction) Execute(ctx context.Context, query string) (*QueryResult, error) {
	...
	if ok != nil {
		...
		case *pb.Query_InitialRes_Ok_Done_:
			// Query completed, no streaming results
			result.IsDone = true
			if res.Done != nil {
		...
		case *pb.Query_InitialRes_Ok_ConceptRowStream_:
			...
			// Continue receiving streaming data
...
// receiveRowStream receives row streaming results
...
// convertConcept converts Concept to generic interface{}
func convertConcept(concept *pb.Concept) interface{} {
	if concept == nil {
		return nil
	}

	// Return corresponding value based on specific concept type
...
// convertDocument converts ConceptDocument to map
...
// convertDocumentNode recursively converts document node
...
// convertRowEntry converts row entry
...
// convertValue converts Value to basic type
...
// convertQueryType converts query type
...
func (tx *Transaction) Rollback(ctx context.Context) error {
	if tx.closed.Load() {
		return nil // Already closed transaction doesn't need rollback
	}
...
// Close closes transaction (WRITE/SCHEMA transactions automatically rollback, READ transactions close directly)
...
// generateRequestID generates unique request ID (must be 16-byte UUID format)
...
// convertTxTypeToPB converts transaction type to protobuf enum
